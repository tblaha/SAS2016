...
} elseif ($_GET['batch_transaction']) {
    $data = str_getcsv($_GET['t_id_batch'], ",");
    $result = batch_transaction($_GET['f_cat'], $f_id, $_GET['t_cat'], $data, $amount, $_GET['tax'], $_GET['description']);
} else {
    $result = revoke_transaction($trans_id, NULL);
}


  echo "Aktion Erfolgreich!<br>";
  if ($_GET['batch_transaction']) {
    echo "Batch : Folgende Überschreibungen wurden im Batch getätigt:<br>";
  } else {
    echo "Single: Folgende Überschreibungen fanden bisher von dem Sender zum Empfänger statt:<br>";
  }
  echo "<table>";
  echo "<tr>";
